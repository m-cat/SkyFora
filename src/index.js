...
var app = new Vue({
  ...
  methods: {
    ...
    shownProfileName(skyforaId) {
      ...
      if (info.data) {
        return info.data.profile.name &&
          info.data.profile.name.trim().length > 0
    ...
    async zoomProfile(skyforaId) {
      ...
      window.location.hash = `${skyforaId}${
        this.zoomedPostId ? ":" + this.zoomedPostId : ""
      }`;
    ...
    getProfileLink(id) {
      ...
      M.toast({
        html: `Shareable profile link copied to clipboard!<br>${link}`,
      });
    },
    ...
    bigpic($event, m, mindex, p) {
      ...
      } else if (m.type.startsWith("text/") || m.type.endsWith("/pdf")) {
        bp({
          el: $event.target,
    ...
    addMedia() {
      this.ui.newPost.files = this.ui.newPost.files.concat([
    ...
    showZoomedProfile() {
      ...
      for (var i = 0; i < pe.length; i++) {
        pdata += `${pe[i][0]}: ${
          pe[i][1].length ? pe[i][1] : "&lt;undefined&gt;"
        }\n`;
      }
    ...
    logoutProfile() {
      ...
      if (
        ...
        if (
          confirm(
            `Do you want to forget these credentials? ( ${this.user.login} )`
          )
        ) {
    ...
    async loadProfile(first) {
      ...
      if (first) {
        if (
          confirm(
            "Do you want to remember these credentials in this device (unsafe) ?"
          )
        ) {
          localStorage.skyforaLogin = this.user.login;
          localStorage.skyforaPassword = this.user.password;
    ...
    async loadWatchlist() {
      ...
      function loadingMsg(skyforaId, url) {
        if (url) {
          self.tickStatus(
            `[${
              wlc + errorCount + 1
            }/${watchlistCount}] Loading ${skyforaId} (${wi})`
          );
        } else {
          self.tickStatus(
            `[${wlc + errorCount + 1}/${watchlistCount}] Loading ${skyforaId}`
          );
        }
      ...
      for (var i = 0; i < w.length; i++) {
        ...
        if (this.isSkyforaId(w[i])) {
          ...
          let res = await this.skyDbRead(
            skyforaIdToPublicKey(w[i]),
            this.conf.channel
          );
          if (res) {
            wlc++;
            ...
            if (xhr.status != 200) {
              ...
              for (var j = 0; j < listIds.length; j++) {
                ...
                if (self.isSkyforaId(listIds[j])) {
                  ...
                  let res = await self.skyDbRead(
                    skyforaIdToPublicKey(listIds[j]),
                    self.conf.channel
                  );
                  if (res) {
                    wlc++;
    ...
    async processSourceBtn() {
      ...
      await xhr.open("GET", this.ui.newSource, true);
      xhr.onload = async () => {
        if (xhr.status != 200) {
          ...
          self.ui.newSourceStatus = `Found ${
            self.matchSkyforaIds(xhr.response).length
          } IDs in URL`;
    ...
    async loadNewSource() {
      let res = await this.skyDbRead(
        skyforaIdToPublicKey(this.ui.newSource),
        this.conf.channel
      );
    ...
    async refreshDisplayPosts(postlist, traceReps) {
      ...
      if (!traceReps) {
        ...
        for (var i = 0; i < allReps.length; i++) {
          dp.push(...(await this.getPostTrace(allReps[i])));
        }
      }
      ...
      var getLatestReplyTime = (list) => {
        var time = new Date(-8640000000000000),
          ltime;
    ...
    isSkyforaId(s) {
      let a = /([ḀḁḂḃḄḅḆḇḈḉḊḋḌḍḎḏḐḑḒḓḔḕḖḗḘḙḚḛḜḝḞḟḠḡḢḣḤḥḦḧḨḩḪḫḬḭḮḯḰḱḲḳḴḵḶḷḸḹḺḻḼḽḾḿṀṁṂṃṄṅṆṇṈṉṊṋṌṍṎṏṐṑṒṓṔṕṖṗṘṙṚṛṜṝṞṟṠṡṢṣṤṥṦṧṨṩṪṫṬṭṮṯṰṱṲṳṴṵṶṷṸṹṺṻṼṽṾṿẀẁẂẃẄẅẆẇẈẉẊẋẌẍẎẏẐẑẒẓẔẕẖẗẘẙẚẛẜẝẞẟẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹỺỻỼỽỾỿ]{32})/.test(
        s
      );
      return a;
    },
    isPostId(s) {
      let a = /([ḀḁḂḃḄḅḆḇḈḉḊḋḌḍḎḏḐḑḒḓḔḕḖḗḘḙḚḛḜḝḞḟḠḡḢḣḤḥḦḧḨḩḪḫḬḭḮḯḰḱḲḳḴḵḶḷḸḹḺḻḼḽḾḿṀṁṂṃṄṅṆṇṈṉṊṋṌṍṎṏṐṑṒṓṔṕṖṗṘṙṚṛṜṝṞṟṠṡṢṣṤṥṦṧṨṩṪṫṬṭṮṯṰṱṲṳṴṵṶṷṸṹṺṻṼṽṾṿẀẁẂẃẄẅẆẇẈẉẊẋẌẍẎẏẐẑẒẓẔẕẖẗẘẙẚẛẜẝẞẟẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹỺỻỼỽỾỿ]{32}:[A-Za-z0-9_-]{24})/.test(
        s
      );
      return a;
    },
    matchSkyforaIds(s) {
      return s.match(
        /([ḀḁḂḃḄḅḆḇḈḉḊḋḌḍḎḏḐḑḒḓḔḕḖḗḘḙḚḛḜḝḞḟḠḡḢḣḤḥḦḧḨḩḪḫḬḭḮḯḰḱḲḳḴḵḶḷḸḹḺḻḼḽḾḿṀṁṂṃṄṅṆṇṈṉṊṋṌṍṎṏṐṑṒṓṔṕṖṗṘṙṚṛṜṝṞṟṠṡṢṣṤṥṦṧṨṩṪṫṬṭṮṯṰṱṲṳṴṵṶṷṸṹṺṻṼṽṾṿẀẁẂẃẄẅẆẇẈẉẊẋẌẍẎẏẐẑẒẓẔẕẖẗẘẙẚẛẜẝẞẟẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹỺỻỼỽỾỿ]{32})/g
      );
    },
    matchPostIds(s) {
      return s.match(
        /([ḀḁḂḃḄḅḆḇḈḉḊḋḌḍḎḏḐḑḒḓḔḕḖḗḘḙḚḛḜḝḞḟḠḡḢḣḤḥḦḧḨḩḪḫḬḭḮḯḰḱḲḳḴḵḶḷḸḹḺḻḼḽḾḿṀṁṂṃṄṅṆṇṈṉṊṋṌṍṎṏṐṑṒṓṔṕṖṗṘṙṚṛṜṝṞṟṠṡṢṣṤṥṦṧṨṩṪṫṬṭṮṯṰṱṲṳṴṵṶṷṸṹṺṻṼṽṾṿẀẁẂẃẄẅẆẇẈẉẊẋẌẍẎẏẐẑẒẓẔẕẖẗẘẙẚẛẜẝẞẟẠạẢảẤấẦầẨẩẪẫẬậẮắẰằẲẳẴẵẶặẸẹẺẻẼẽẾếỀềỂểỄễỆệỈỉỊịỌọỎỏỐốỒồỔổỖỗỘộỚớỜờỞởỠỡỢợỤụỦủỨứỪừỬửỮữỰựỲỳỴỵỶỷỸỹỺỻỼỽỾỿ]{32}:[A-Za-z0-9_-]{24})/g
      );
    },
    getPosts(filterStr) {
      let r = [];
    ...
    getProfileInfo(skyforaId) {
      ...
      if (this.userWatchlistUrlLists[skyforaId]) {
        ...
        if (this.dataCache[skyforaId] !== undefined) {
          ...
          var self = this;
          return this.skyDbRead(
            skyforaIdToPublicKey(skyforaId),
            this.conf.channel
          ).then((res) => {
            if (res !== null) {
